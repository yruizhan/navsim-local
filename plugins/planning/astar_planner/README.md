# AstarPlanner Planner Plugin

A* path planner with occupancy grid map
...
#### 2. 更新适配层头文件 (`adapter/astar_planner_plugin.hpp`)
...
#### 3. 更新适配层实现 (`adapter/astar_planner_plugin.cpp`)
...
cd build
cmake ..
make astar_planner_plugin -j4
./navsim_local_debug --planner AstarPlanner --scenario scenarios/simple_corridor.json
```
...
编辑 `algorithm/astar_planner.cpp` 中的 `plan()` 方法：
...
在 `algorithm/astar_planner.hpp` 的 `Config` 结构体中添加参数：
...
在 `adapter/astar_planner_plugin.cpp` 的 `loadConfig()` 中解析：
