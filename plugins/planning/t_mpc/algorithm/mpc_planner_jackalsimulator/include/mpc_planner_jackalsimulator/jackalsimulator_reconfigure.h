...
#include <mpc_planner_jackalsimulator/jackalsimulatorConfig.h>

class JackalsimulatorReconfigure
{
public:
	JackalsimulatorReconfigure()
	{
		...
		ros::NodeHandle nh_reconfigure("mpc_planner_jackalsimulator");
		_reconfigure_server.reset(new dynamic_reconfigure::Server<mpc_planner_jackalsimulator::jackalsimulatorConfig>(_reconfig_mutex, nh_reconfigure));
		_reconfigure_server->setCallback(boost::bind(&JackalsimulatorReconfigure::reconfigureCallback, this, _1, _2));
	}
	void reconfigureCallback(mpc_planner_jackalsimulator::jackalsimulatorConfig &config, uint32_t level)
	{
		(void)level;
...
private:
	...
	boost::shared_ptr<dynamic_reconfigure::Server<mpc_planner_jackalsimulator::jackalsimulatorConfig>> _reconfigure_server;
